...
const MatchCard = ({ match, tournament, onUpdate, onUpdateScore }) => {
  ...
  const handleSaveScore = (scoreDetails) => {
    onUpdateScore({ scoreDetails, status: 'completed' });
  };
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex items-center space-x-4 min-w-[200px]">
          ...
          <div>
            ...
            <span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${match.status === 'completed' ? 'bg-green-50 text-green-700 border-green-100' :
      ...
      {isEditing && (
        <div className="mt-6 pt-6 border-t border-slate-100 animate-fade-in">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Score Edit (Only if teams exist) - Sport-Specific */}
            {match.homeTeam && match.awayTeam && (
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                <h5 className="text-sm font-bold text-slate-900 mb-3 flex items-center">
